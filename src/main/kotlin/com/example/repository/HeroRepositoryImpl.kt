...
class HeroRepositoryImpl : HeroRepository {
    ...
    override suspend fun getAllHeroes(page: Int): ApiResponse {
        return ApiResponse(
            success = true,
            message = "OK",
            prevPage = calculateHeroes(page)[PREVIOUS_PAGE_KEY],
            nextPage = calculateHeroes(page)[NEXT_PAGE_KEY],
            heroes = heroes[page]!!,
    ...
    private fun calculateHeroes(page: Int): Map<String, Int?> {
        var prevPage: Int? = page
        var nextPage: Int? = page

        if (page in 1..4) nextPage?.plus(1)
        if (page in 2..5) prevPage?.minus(1)
